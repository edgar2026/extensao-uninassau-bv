...
const PAGE_CONFIG = {
  first_access: {
    title: 'Criar ou recuperar senha com código',
    subtitle: 'Informe seu e-mail, o código recebido e crie ou recupere sua senha.',
    codeLabel: 'Código de acesso',
    submitLabel: 'Criar nova senha e acessar',
    submittingLabel: 'Processando...',
    successMessage: 'Senha criada com sucesso. Faça login para acessar.',
  },
  password_reset: {
    title: 'Criar ou recuperar senha com código',
    subtitle: 'Informe seu e-mail, o código recebido e crie ou recupere sua senha.',
    ...
    submitLabel: 'Criar nova senha e acessar',
    submittingLabel: 'Processando...',
    successMessage: 'Senha redefinida com sucesso. Faça login para acessar.',
  },
} as const;

type Purpose = keyof typeof PAGE_CONFIG;

export const CodigoSenhaPage: React.FC = () => {
  const navigate = useNavigate();
  const [searchParams] = useSearchParams();
  const rawPurpose = searchParams.get('purpose');
  const purpose: Purpose = rawPurpose === 'password_reset' ? 'password_reset' : 'first_access';
  ...
  const onSubmit = async (data: CodeAndPasswordInputs) => {
    ...
    try {
      ...
      navigate('/', { state: { message: config.successMessage } });
    } catch {
          ...
          >
            ...
            <form onSubmit={handleSubmit(onSubmit)} className="flex flex-col gap-4 mt-6">
              <div className="auth-field">
                <label className="login-montserrat" style={labelStyle}>E-mail cadastrado</label>
                <Input
              ...
              <div className="auth-field">
                <label className="login-montserrat" style={labelStyle}>{config.codeLabel}</label>
                <Input
